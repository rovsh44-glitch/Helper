...
internal static partial class RuntimeLogSemanticDeriver
{
    ...
    private static string InferScope(string lower, string severity)
    {
        ...
        if (Regex.IsMatch(lower, @"\b(snapshot|journal|persist|queue|database|storage|disk)\b"))
        {
            return "storage";
        }
    ...
    private static string InferDomain(string lower, string scope)
    {
        ...
        if (scope == "storage" || Regex.IsMatch(lower, @"\b(snapshot|journal|persist|queue flush|journal write)\b"))
        {
            return "persistence";
        }
        ...
        if (scope is "api" or "network" || Regex.IsMatch(lower, @"\b(http|transport|endpoint|request|response|socket)\b"))
        {
            return "transport";
        }
    ...
    private static string InferOperationKind(string lower, string sourcePath, int? latencyMs, string? route)
    {
        ...
        if (Regex.IsMatch(lower, @"\b(snapshot|journal|flush|persist|database)\b"))
        {
            return "persistence";
        }
        ...
        if (sourcePath.Contains("index", StringComparison.OrdinalIgnoreCase))
        {
            return "indexing";
        }
    ...
    private static IReadOnlyList<string> BuildMarkers(string message, int? latencyMs, string? route, string? correlationId, string? degradationReason, bool isContinuation)
    {
        ...
        var jsonKeys = JsonKeyRegex().Matches(message)
            ...
            .Take(3);
        markers.AddRange(jsonKeys.Select(key => $"json:{key.ToLowerInvariant()}"));
        ...
        return markers.Count == 0 ? Array.Empty<string>() : markers.Distinct(StringComparer.OrdinalIgnoreCase).ToArray();
    }
    ...
    private static string BuildSummary(string message)
    {
        ...
        var summary = message.Replace('\t', ' ').Trim().Replace("  ", " ");
        return summary.Length <= 140 ? summary : summary[..140].TrimEnd() + " ...";
    }
