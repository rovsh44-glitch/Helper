...
public static class BackendEnvironmentInventory
{
    ...
    private static void AddResearchDefinitions(ICollection<BackendEnvironmentVariableDefinition> definitions)
    {
        ...
        definitions.Add(Def("HELPER_RESPONSE_SHOW_LOCAL_PATHS", "Research", "backend_runtime", "bool", "Expose absolute local library paths in local source labels. Defaults to false so public proof artifacts remain path-redacted.", defaultValue: "false", consumers: Values("src/Helper.Runtime/LocalLibraryGroundingSupport.cs")));
        definitions.Add(Def("HELPER_LOCAL_LIBRARY_PUBLIC_SOURCE_MODE", "Research", "backend_runtime", "enum", "Local-library source rendering mode for public artifacts.", defaultValue: "redacted", allowedValues: Values("redacted", "developer"), consumers: Values("scripts/analyze_local_first_librarian_run.ps1", "src/Helper.Runtime/LocalLibraryGroundingSupport.cs")));
        definitions.Add(Def("HELPER_LOCAL_LIBRARY_ALLOW_CURRENT_FACTS", "Research", "backend_runtime", "bool", "Allow explicitly trusted local-library documents to support current external facts. Defaults to false; web remains required for fresh/regulatory claims.", defaultValue: "false", consumers: Values("src/Helper.Api/Conversation/EvidenceFusionService.cs", "src/Helper.Api/Conversation/Epistemic/BehavioralCalibrationPolicy.cs")));
        definitions.Add(Def("HELPER_LOCAL_LIBRARY_CURRENT_FACT_ALLOWLIST", "Research", "backend_runtime", "csv", "Optional stable source IDs allowed by operator policy to support current-fact claims.", consumers: Values("src/Helper.Api/Conversation/EvidenceFusionService.cs")));
        definitions.Add(Def("HELPER_MIXED_EVIDENCE_REQUIRE_WEB_FOR_FRESH", "Research", "backend_runtime", "bool", "Require web evidence for freshness-sensitive claims even when local library evidence is available.", defaultValue: "true", consumers: Values("src/Helper.Api/Conversation/EvidenceFusionService.cs", "src/Helper.Api/Conversation/Epistemic/BehavioralCalibrationPolicy.cs")));
    ...
    private static void AddKnowledgeAndIndexingDefinitions(ICollection<BackendEnvironmentVariableDefinition> definitions)
    {
        ...
        definitions.Add(Def("HELPER_PDF_VISION_GHOSTSCRIPT_PATH", "Knowledge And Indexing", "backend_runtime", "path", "Explicit Ghostscript executable for PDF-to-image vision extraction.", consumers: Values("src/Helper.Runtime.Knowledge/StructuredPdfParser.Vision.cs")));
        definitions.Add(Def("HELPER_PDF_VISION_GHOSTSCRIPT_DPI", "Knowledge And Indexing", "backend_runtime", "int", "Ghostscript raster DPI for PDF vision fallback.", defaultValue: "200", minValue: "96", maxValue: "600", consumers: Values("src/Helper.Runtime.Knowledge/StructuredPdfParser.Vision.cs")));
        definitions.Add(Def("HELPER_PDF_VISION_GHOSTSCRIPT_JPEG_QUALITY", "Knowledge And Indexing", "backend_runtime", "int", "Ghostscript JPEG quality for PDF vision fallback images.", defaultValue: "80", minValue: "40", maxValue: "95", consumers: Values("src/Helper.Runtime.Knowledge/StructuredPdfParser.Vision.cs")));
        definitions.Add(Def("HELPER_VISION_OCR_MAX_IMAGE_BYTES", "Knowledge And Indexing", "backend_runtime", "int", "Maximum encoded image bytes accepted by vision OCR preparation.", defaultValue: "12582912", minValue: "1024", maxValue: "67108864", consumers: Values("src/Helper.Runtime.Knowledge/VisionImagePreparation.cs")));
        definitions.Add(Def("HELPER_VISION_OCR_TIMEOUT_SEC", "Knowledge And Indexing", "backend_runtime", "int", "Vision OCR extraction timeout in seconds.", defaultValue: "90", minValue: "10", maxValue: "600", consumers: Values("src/Helper.Runtime.Knowledge/StructuredPdfParser.Vision.cs")));
