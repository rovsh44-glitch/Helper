...
if ([string]::IsNullOrWhiteSpace($ApiKey)) {
    throw "[PendingExtBatch] HELPER_API_KEY is required."
}
...
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    ...
    $ReportPath = Join-Path $pathConfig.OperatorRuntimeRoot ("pending_" + $Extension.TrimStart('.') + "_batch_" + $stamp + ".md")
}

if ([string]::IsNullOrWhiteSpace($ActivePidPath)) {
    $ActivePidPath = Join-Path $pathConfig.OperatorRuntimeRoot ("pending_" + $Extension.TrimStart('.') + "_batch_active.pid")
}
...
function Get-PendingFilesByExtension {
    $queue = Read-Queue
    return @(
        $queue.GetEnumerator() |
            Where-Object {
                [string]$_.Value -eq "Pending" -and
                [System.IO.Path]::GetExtension([string]$_.Key).Equals($Extension, [System.StringComparison]::OrdinalIgnoreCase)
            } |
            Sort-Object Key |
...
$files = Get-PendingFilesByExtension
Add-Content -Path $ReportPath -Value ("- PendingFilesAtStart: {0}" -f @($files).Count) -Encoding UTF8
