...
public sealed class MethodSignatureNormalizer
{
    private static readonly Regex MultiSpace = new(@"\s+", RegexOptions.Compiled);
    private readonly MethodSignatureValidator _validator;

    public MethodSignatureNormalizer(MethodSignatureValidator validator)
    {
    ...
    private static bool TryNormalizeForInterface(string signature, out string? normalized, out string? error)
    {
        ...
        var cleanedModifiers = method.Modifiers.Where(m => !m.IsKind(Microsoft.CodeAnalysis.CSharp.SyntaxKind.AsyncKeyword)).ToArray();
        var cleaned = method
            .WithAttributeLists(default)
            .WithBody(null)
            .WithExpressionBody(null)
            .WithModifiers(new Microsoft.CodeAnalysis.SyntaxTokenList(cleanedModifiers))
