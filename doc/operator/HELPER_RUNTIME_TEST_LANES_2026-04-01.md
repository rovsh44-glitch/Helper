...
3. `Certification`
   Run for nightly, release, parity, benchmark, and heavy regression validation.
   Entry point: `scripts/run_certification_tests.ps1`
   Scope: parity, benchmark, lifecycle, routing, diagnostics, and certification history coverage that does not spawn nested `dotnet`

4. `CertificationCompile`
...
- `Certification` owns slower eval, parity, lifecycle, routing, and template regression coverage that stays in-process.
